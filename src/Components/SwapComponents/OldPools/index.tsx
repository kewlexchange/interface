...
import { useDiamondContract, useOldExchangeContract, useERC20Contract, usePAIRContract } from '../../../hooks/useContract';
...
const _OLD_POOL_TAB = () => {
    ...
    const EXCHANGE = useOldExchangeContract(chainId, true)
    ...
    const handleRemoveLiquidity = async () => {
        ...
        if (balance.gt(0)) {
            ...
            if ((pairInfo.base.token === WETH9[chainId].address) || (pairInfo.quote.token === WETH9[chainId].address)) {
                ...
                await EXCHANGE.removeLiquidityETH(tokenAddress, liquidity, amountAMin, amountBMin, to, deadline).then(async (tx) => {
                    await tx.wait();
                ...
                await EXCHANGE.removeLiquidity(pairInfo.base.token, pairInfo.quote.token, liquidity, amountAMin, amountBMin, to, deadline).then(async (tx) => {
                    await tx.wait();
                    const summary = `Remove Liquidity: ${tx.hash}`
                    setTransaction({ hash: tx.hash, summary: summary, error: null });
                    toggleTransactionSuccess();
    ...
    useEffect(() => {
        ...
        setQuoteAsset(defaultAssets.find(token => token?.symbol === "IMON"))
    ...
    const handleSelectPair = (base: any, quote: any) => {
        setBaseAsset(base);
        setQuoteAsset(quote);
        toggleSelectToken();
    }
...
export const OLD_POOL_TAB = memo(_OLD_POOL_TAB)
